...
export default function VisionTherapyPage() {
  ...
  const finishTherapy = async () => {
    ...
    // To follow GameSession model (metrics.vision), include therapy results under metrics.vision
    const sessionData = {
      diseaseType: "vision",
      mode: "therapy",
      result: "completed",
      metrics: {
        vision: {
          ...
          time: null, // we don't track total seconds here in this therapy flow; keep null if not computed
          finalThreshold: difference, // extra field included under vision metrics — harmless for DB if schema allows it
        },
      },
    };
      ...
      <div className="flex flex-col items-center justify-start min-h-screen bg-black text-white p-6 pt-20">
        ...
        <div className="flex items-center justify-between gap-10 mt-6">
          <button
            ...
            className="p-6 bg-blue-900 hover:bg-blue-800 rounded-xl transition-transform hover:scale-105"
          >
            <div
              style={{
                width: leftValue,
