...
// SVG renderer for shapes (with simple 3D-ish effect)
...
export default function VisionTestPage() {
  ...
  const isPostingRef = useRef(false); // same pattern as Parkinsons guarded version
    ...
    setScore((prev) => {
      ...
      setTimeout(() => {
        if (nextIndex >= TOTAL_TRIALS) {
          finishRun(newScore); // pass score directly
        } else {
  ...
  // finishRun no longer posts — it only sets finished/result/endTime (POST happens in useEffect below)
  ...
  // POST when the run finishes — same pattern as Parkinsons: useEffect + isPostingRef guard
  useEffect(() => {
    if (!finished || !result) return;

    // guard identical to Parkinsons implementation to avoid double saves
    ...
    axios
      ...
      .then(() => {
        // optional: console.log for parity with Parkinsons
      ...
      .finally(() => {
        // release guard after short delay — same behavior as Parkinsons guard
        setTimeout(() => {
  ...
  if (started && !finished) {
    ...
    return (
      <div className="flex flex-col items-center justify-start min-h-screen bg-black text-white p-6 pt-24">
        ...
        <div className="mb-4 text-gray-300 text-xl">
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-black text-white p-6">
        ...
        {(result.status === "Yellow" || result.status === "Red") && (
          <button className="max-w-[250px] bg-yellow-500 hover:bg-yellow-400 text-black px-6 py-3 rounded-lg text-xl font-semibold mb-4" onClick={() => window.location.href = "/therapy/vision"}>
